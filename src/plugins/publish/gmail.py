...
class Plugin(PublishPlugin):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.from_name = self.config.get("from_name", "")  # 送信老E��
...
""")
    ...
    def execute(self, entries: Iterator[Entry]):
        ...
        """
        if not self.enabled:
            logger.info("Gmail plugin is disabled. Skipping...")
            print("\nGmail plugin is disabled. Skipping...")
            return
        ...
        try:
            # SMTP 接綁E            server = smtplib.SMTP(self.smtp_server, self.smtp_port)
            server.ehlo()
            server.starttls()
            server.ehlo()
            ...
            for entry in entries_list:
                ...
                # 送信老E��を設定（例：「秘書メイ <email@example.com>」！E                if self.from_name:
