...
class Plugin(SubscriptionPlugin):
    ...
    Fetches from the search results page for "逋ｺ陦ｨ莨・ keyword.
    """
    ...
    def _fetch_press_releases(self) -> Iterator[Dict[str, str]]:
        ...
        try:
            ...
            # BeautifulSoup 縺ｧ繝代・繧ｹ
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(html, 'html.parser')

            # 繝励Ξ繧ｹ繝ｪ繝ｪ繝ｼ繧ｹ繝ｪ繝ｳ繧ｯ繧貞叙蠕・            links = soup.select('a[href*="/main/html/rd/p/"]')
            ...
            for link in links:
                ...
                # 繧ｵ繝槭Μ繝ｼ逕滓・
                summary = f"{title} 縺ｮ繝励Ξ繧ｹ繝ｪ繝ｪ繝ｼ繧ｹ縺悟・髢九＆繧後∪縺励◆縲・
    ...
    def execute(self) -> Iterator[Entry]:
        ...
        print(f" 笨・Fetched {count} new press release(s) from PR Times")
        self._save_state()
